...
def get_path(wire, centre):
    ...
    for run in wire:
        ...
        path += position.get_line_to(new_position)
...
def get_intersections(wire_1, wire_2, centre=(0, 0)):
    ...
    return set(wire_1_path).intersection(wire_2_path)


def find_closest(intersections, centre=(0, 0)):
    """
    Finds closest intersection to the centre based on Manhattan distance
    """
    closest = None
    min_dist = None
    ...
    for intersection in intersections:
        ...
        distance = abs(intersection[0] - centre[0]) + \
            abs(intersection[1] - centre[1])
...
if __name__ == "__main__":
    ...
    print(f"Result (coordinate, distance): {find_closest(intersections)}")
